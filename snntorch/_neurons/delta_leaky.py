...
class Delta_Leaky(LIF):
    ...
    * :math:`β` - Membrane potential decay rate
    ...
    def __init__(
        ...
        learnable_delta_threshold=False,
        ...
        super(Delta_Leaky, self).__init__(
            beta,
            threshold,
            spike_grad,
            surrogate_disable,
        ...
        if learnable_delta_threshold:
            delta_threshold = torch.Tensor([delta_threshold])
    ...
    def sparsity_loss(self):
        zero_count = torch.sum(self.delta_matrix == 0).item()
        total_elements = self.delta_matrix.numel()
    ...
    def _base_state_function_hidden(self, input_):
        ...
        delta = self.mem - (self.beta.clamp(0, 1) * self.mem + input_)
        self.delta_matrix = torch.where(torch.abs(delta) > self.delta_threshold, delta, 0)
        base_fn = self.mem - self.delta_matrix
    ...
    def detach_hidden(cls):
        ...
        for layer in range(len(cls.instances)):
            if isinstance(cls.instances[layer], Delta_Leaky):
